    ...
    * an optional suffix for IPV6 addresses from this list: `eui-64`, `link-local`
    ...
    validate do |values|
      ...
      values.each do |value|
        raise _('Invalid interface ip address') unless parse(value.gsub(%r{\s*(eui-64|link-local)\s*$}, ''))
      end
    end

    munge do |value|
      option = (value =~ %r{eui-64|link-local}i) ? value.gsub(%r{^.*?\s*(eui-64|link-local)\s*$}, '\1') : nil
      [parse(value.gsub(%r{\s*(eui-64|link-local)\s*$}, '')), option].flatten
    end
